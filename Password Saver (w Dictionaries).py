"""
Updated password saver that uses dictionaries
William Alber 11/10/2019
"""
import pickle
import sys

# The password list - We start with it populated for testing purposes
entries = {'yahoo': {'username': 'walber', 'password': 'XqffoZeo', 'url': 'https://www.yahoo.com'},
           'google': {'username': 'walber', 'password': 'CoIushujSetu', 'url': 'https://www.google.com'}}
...
def add_entry(website, url, username, password):
    ...
    """
    ciphertext = password_encrypt(password, encryption_key)
    new_entry = {website.lower(): {"username": username, "password": ciphertext, "url": url}}
    entries.update(new_entry)


def lookup_password(website):
    """ Lookup the information for a given website
    :param website (string) The website for the entry to lookup
    :return: Returns website information. Returns None if no entry is found
    """
    cipher = entries[website]["password"]
    plain = password_decrypt(cipher, encryption_key)
    print("\nUsername: ", entries[website]["username"])
    print("Password: ", plain)
    print("URL: ", entries[website]["url"])
    return plain
...
while True:
    ...
    if choice == '3':  # Add a new entry
        ...
        add_entry(website, url, username, unencrypted_password)
